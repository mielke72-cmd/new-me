...
// strength sessions (set/rep cues stay constant; intensity comes from the block note)
const dayA = {
...
export default function App() {
  ...
  useEffect(() => {
    if (!loaded) return;
    (async () => {
      try { if (window.storage) await window.storage.set("progress", JSON.stringify(done)); } catch (e) {}
    })();
  }, [done, loaded]);
  ...
  // overall progress (sessions = days fully checked)
  const stats = useMemo(() => {
  ...
  return (
    <div className="app">
      ...
      <div className="block-banner" style={{ borderColor: "var(--lime)" }}>
...
.block-banner { background: var(--surf); border-left: 3px solid; border-radius: 0 10px 10px 0; padding: 11px 14px; margin-bottom: 16px; }
